...
3. **Sentinel-Gemma catches it (45s):** compound risk climbs; lead-time forecast: *"threshold crossing predicted in ~18 min."*
...
7. **Scoreboard (30s):** side-by-side baseline vs Sentinel-Gemma — false negatives, false alarms, lead time. The slide that wins.
...
- Day 10–11: **baseline-vs-Sentinel-Gemma evaluation harness** (produces the headline chart — highest priority).
...
Most teams build "a dashboard that shows sensor values and an alert." Sentinel-Gemma is the only entry that shows a **controlled comparison** (baseline vs compound), a **quantified lead time**, an **agent that acts** (not just displays), and **cited regulation** — mapping directly to Innovation (25) + Business Impact (25) + the exact evaluation metrics.
